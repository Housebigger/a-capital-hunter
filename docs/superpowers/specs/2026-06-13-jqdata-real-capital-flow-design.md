...
JQData 账号权限可能限制可查询日期范围。采集器必须以接口实际返回和账号权限为准，不能把较早日期伪装成“最新”；若试用权限无法访问最近交易日，日终目标需要开通对应权限后才能通过真实链路验收。
...
- 源字段单位：万元。
- 入库单位：人民币元（CNY）；JQData 适配器在边界处将源值乘以 `10_000`。
...
interface StockCapitalFlowPoint {
  ...
  subThemeId: string;
  themeId: string;
  aggregationRole: "primary" | "related";
  netAmountMain: number;
  tradeDate: string;
}
```

`points` 可为同一证券包含多个展示映射，但必须携带相同的原子资金值。`aggregationRole` 明确标识唯一主归属与关联展示；所有向上聚合都先以 `securityCode` 识别证券，不能把重复展示记录当作多笔资金流。
