...
> **Current state:** data pipeline + frontend work end-to-end (the earlier "frontend stuck on 等待真实资金流快照" blocker was root-caused and fixed). Since then: content depth expanded (SP1 → 74 sub-themes / 408 stocks), the heat-driven dynamic layout shipped (SP2), a static-site deployment path was added, and the mobile/responsive pass shipped (SP3 — a single ≤900px "compact" mode: sticky control bar, two-finger-rotate / one-finger-scroll touch, dpr/shadow/label-density tuning; desktop unchanged), and the UI/interaction polish pass shipped (SP4 — real cell selection across P1/P2/P3 with a gold ring + emissive + inspector detail; the honesty fix that real data is labeled 主力净流入 not 模拟; list/transition/typography polish), and the share/SEO meta pass shipped (SP5 — Open Graph/Twitter tags + favicon + 1200×630 share image + robots; honesty-framed description). **All five pre-publish sub-projects (SP1–SP5) are done; the site is publish-ready** — it goes live once the user enables GitHub Pages and adds the `TUSHARE_TOKEN` Actions secret (the deploy workflow then syncs, builds, and publishes to `https://housebigger.github.io/a-capital-hunter/`). See `STATUS.md` for the full handoff.
...
- Frontend: Vitest globals (`describe`/`it`/`expect`, no imports); component tests use Testing Library + jsdom; `vitest.config` excludes `node_modules`, `dist`, `.worktrees/**`, `tests/e2e/**`. ~218 tests.
