...
JQData/Tushare 真实资金流管线已**全部实现并验证可用**，真实数据已成功入库（2026-06-17，167/168 只股票）。前端卡点（「一直等待真实资金流快照」）的**根因已定位并修复**（见下「✅ 卡点根因与修复」），数据层已端到端验证、**浏览器已确认渲染正常**。在此之上完成一轮 **UI 优化 + 资金流时间档位（P0–P5）**，已合并入 `main`（见下「✅ UI 优化 + 资金流时间档位」）。此后又完成一批**发布前优化**——窗口 bug 修复、仓库清理 + 推送 GitHub、中英文 README、静态站点部署、**SP1 内容深度（子题材 45→74、个股 184→408）**、**SP2 热度驱动动态布局**、**SP3 移动端/响应式**、**SP4 界面/交互打磨**、**SP5 分享/SEO 元信息**——均已合并入 `main`（见下「✅ 发布前优化 + 静态部署」）。**SP1–SP5 全部完成，站点已就绪，待用户启用 GitHub Pages + 配置 `TUSHARE_TOKEN` secret 后自动部署。**
...
- **SP5 — 分享/SEO 元信息**（merge `461b130`）：`index.html` 加标题/描述（**诚实口径：数据可视化·非投资建议**）/theme-color/canonical + 完整 Open Graph + Twitter `summary_large_image`；`public/`：`favicon.svg`（流入/流出柱形 motif）、`apple-touch-icon.png`、**1200×630 og-image**（取自 3D 地图截图，sips 生成）、`robots.txt`。爬虫 URL 用绝对地址（`housebigger.github.io/a-capital-hunter/`），图标用 `%BASE_URL%` 适配子路径；含 index.html meta 存在性测试守卫。纯静态/表现层。门禁：前端 **218**、后端 90、tsc 0、build:pages OK（已验证 dist 内 `%BASE_URL%` 解析 + 资源就位）。

**发布前优化全部完成（SP1–SP5）。** 站点**待用户操作上线**：① 在 repo Settings → Pages 选 GitHub Actions 源；② 加 `TUSHARE_TOKEN` Actions secret。之后 deploy 工作流自动同步 20 日数据 → 构建 → 发布到 `https://housebigger.github.io/a-capital-hunter/`，再用 opengraph.xyz 之类做分享卡片复核。
